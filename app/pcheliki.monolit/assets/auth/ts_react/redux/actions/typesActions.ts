export const NOT_LOADED_SELECT_COUNTRIES: string = 'NOT_LOADED_SELECT_COUNTRIES';

export const LOADED_SELECT_COUNTRIES: string = 'LOADED_SELECT_COUNTRIES';

export const SET_METHOD_AUTH: string = 'SET_METHOD_AUTH';

export const SET_COUNTRY: string = 'SET_COUNTRY';

export const SET_PHONE: string = 'SET_PHONE';

export const SET_REMEMBER_ME: string = 'SET_REMEMBER_ME';

export const SET_QR_CODE: string = 'SET_QR_CODE';

export const LOADED_QR_CODE: string = 'LOADED_QR_CODE';

export const NOT_LOADED_QR_CODE: string = 'NOT_LOADED_QR_CODE';